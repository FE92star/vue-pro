...
nav.install = function (Vue, options) {
  ...
  let $vNav = new NavTpl()
  const PAGE = window.navRouterPageQuery || 'page' // 使用连接参数page作为history的参数
  window.vNav = $vNav
  ...
  window.onpopstate = function (event) {
    try {
      ...
      let queryParams = o[PAGE] ? o[PAGE] : 'ORIGIN'
  ...
  Vue.prototype.$nav = {
    ...
    /**
     * exp: this.$nav.navListen('home', 'back', ()=> {console.log("hello")})
     * @param {*} pageId 页面的id值   同一个id且同一个name若有多个函数 会并发执行
     * @param {*} name 监听事件的tag值 如back等
     * @param {*} fn 监听tag所属的函数
     */
