...
<template lang="html">
  <div class="progress-wrap">
    <!-- 线型进度条 -->
    <div class="progress-line" :style="lineStyle" v-if="type === 'line'">
      <div class="progress" :style="barStyle">
        <div class="progress-text" v-if="textShow" :style="{lineHeight: `${strokeHeight}px`}">{{percentage}}%</div>
      </div>
    </div>
    <!-- 圆形进度条 -->
    <div class="progress-circle" v-else-if="type === 'circle'">
      ...
      <div class="circle-text" v-if="circleText" :style="{color: `${textColor}`}">{{circleText}}</div>
    </div>
  </div>
</template>
<script>
...
<style lang="less">
  .progress-wrap { //条形进度条以父元素的宽高作为标准
    position: relative;
    width: 100%;
    height: 100%;
    .progress-line {
