...
<div class="page__wrap">
  <transition-group name='fade' tag='div'>
    ...
    <div class="pageWrap_wrap" v-show="(state=='loading')" :style="{background: bgLoad}" :key="1">
      <slot name='loading'>
        <loading-page :stroke='stroke'>
          <!-- <img src="//r.51gjj.com/resource/wealthy/201810_licai_loading.gif" alt="loading" class="load_gif">
          <p class="load_title">页面拼命加载中...</p> -->
        </loading-page>
      </slot>
    </div>
    <div class="pageWrap_wrap" v-show="(state=='fail' || state=='error')" :style="{background: '#fff'}" :key="2">
      ...
      <div class="pageWrap_fail_footer">
        {{failFooterTips}}
      </div>
    </div>
  </transition-group>
